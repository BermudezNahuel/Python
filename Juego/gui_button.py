...
class Button_level(Widget):
    def __init__(self,master,x,y,w,h,color_background,color_border,on_click,on_click_param,data_level,text,font,font_size,font_color):
        super().__init__(master,x,y,w,h,color_background,color_border)
        pygame.font.init()
        self.on_click = on_click
        self.on_click_param = on_click_param
    ...
    def update(self,lista_eventos):
        for evento in lista_eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN:
                if(self.slave_rect_collide.collidepoint(evento.pos)):
                    self.on_click(self.on_click_param,self.data_level)
