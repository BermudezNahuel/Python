...
class FormManager:
    def __init__(self,screen) -> None:
        ...
        self.master = FormPlay(name="master",master_surface = screen,x=0,y=0,w=ALTO_VENTANA,h=ANCHO_VENTANA,color_background=(255,255,255),color_border=(255,0,255),active=False)
        ...
        self.you_lose = FormLose(name="you_lose",master_surface = screen,x=ANCHO_VENTANA//3,y=ALTO_VENTANA//3,w=ANCHO_VENTANA//2,h=ANCHO_VENTANA//1.5,color_background=WHITE,color_border=WHITE,active=False)
    ...
    def Gestionar_formularios(self,lista_eventos,delta_ms,keys):
        ...
        if(self.pantalla_ingreso.active):
            ...
            self.master.reinicio = True
                   
        elif(self.master.active):
            self.master.update(lista_eventos,delta_ms,keys)
            self.master.draw()
